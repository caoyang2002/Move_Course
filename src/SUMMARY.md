# Rust 语言圣经

[关于本书](about-book.md)
[进入 Move 编程世界](into-move.md)
[避免从入门到放弃](first-try/sth-you-should-not-do.md)
<!-- [快速查询入口](index-list.md) 暂时屏蔽-->
[社区](community.md)


# Rust 语言基础学习

---

- [浅尝 Move](first-try/intro.md)
  - [安装 Aptos CLI](first-try/installation.md)
  - [IDE？ VSCode！](first-try/editor.md)
  - [各大 Move 流派](first-try/move-self.md)

- [Move 基础入门](basic/intro.md)
  - [认识 Move 命令行工具](first-try/move-cli.md)
  - [Hello,Blockchain](first-try/hello-blockchain.md)
  - [模块和脚本](basic/module-script.md)

  - [变量绑定与解构](basic/variable.md)
  - [原始类型](basic/primitive-types/intro.md)
    - [整数](basic/primitive-types/integers.md)
    - [地址](basic/primitive-types/address.md)
    - [向量](basic/primitive-types/vector.md)
    - [签名](basic/primitive-types/signer.md)
    - [引用](basic/primitive-types/references.md)
    - [元组与单元](basic/primitive-types/tuple-unit.md)

  - [基本概念](basic-concepts/intro.md)

    - [Move.toml 格式讲解](basic/move-toml.md)


# Rust 语言进阶学习

---



- [Move 高级进阶](advance/intro.md)


- [源码解析](source-analyze/intro.md)

- [附录：Toml 文件格式](appendix/toml.md)